...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='文章缓存失效 Transactional Outbox';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='点赞关系事实表，interaction 服务所有';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='点赞事件 Transactional Outbox';

CREATE TABLE `like_count_delta` (
    `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    `event_id` CHAR(36) NOT NULL,
    `article_id` BIGINT UNSIGNED NOT NULL,
    `delta` TINYINT NOT NULL COMMENT '1 或 -1',
...
CREATE TABLE `comment` (
    ...
    `article_id` BIGINT UNSIGNED NOT NULL,
    `user_id` BIGINT UNSIGNED NOT NULL,
    `parent_id` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '父评论 id，0 表示顶级评论',
...
-- 本地演示账号密码均为 password；数据库只保存 bcrypt 哈希。

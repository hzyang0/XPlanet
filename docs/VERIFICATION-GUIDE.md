...
7. **4:40～5:00 可靠性证据**：展示 [`evaluation-results.md`](evaluation-results.md) 与 [`EXPERIMENTS.md`](EXPERIMENTS.md)，说明 Outbox/MQ 暂停和 checkpoint 强退测试均可单独复现。

## 扩展恢复验证

下面两项不计入 5 分钟巡检，分别验证 Broker 暂停后的 Outbox 补发，以及 Agent 进程退出后的 checkpoint 恢复：

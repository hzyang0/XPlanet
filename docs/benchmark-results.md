...
> 历史记录：以下数据来自旧的 Redis SADD/Hash 点赞链路，不代表当前 Outbox + 持久化投影实现。
> 其中点赞脚本只使用一个用户重复请求，主要测到幂等 no-op 的短路径，不能作为完整点赞吞吐或削峰倍数。
> 新架构数据补测前，简历和面试不得引用这些数字作为当前性能结论。
...
历史结论仅说明旧实现对单用户重复请求进行了幂等拦截；它没有覆盖大量用户真实状态变化、
Outbox 写入、MQ relay、消费去重和计数投影，不能验证当前完整链路吞吐。

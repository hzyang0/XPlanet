...
POST /api/ai/tasks
  ...
  → Agent 执行 ValidateInput → Planner → [Decide → Search/Fetch → Evidence] 循环 → Writer(Claim) → Critic
  → Critic 通过则 Finalize；关键缺口在剩余预算内最多补研究一次，再重写和复核
...
- Evidence 片段独立保存 locator 与 SHA-256；结果落库前检查 `sourceRef`、`evidenceRef`、Claim 和 Citation 闭包，孤立引用或错误哈希不会进入数据库；
- Writer 必须把 Claim ID 和 Evidence ID 显式写入报告；结构化 Critic 返回缺证据、冲突、错误引用、不确定项和可选补研究 query，工作流硬限制最多一次修订；

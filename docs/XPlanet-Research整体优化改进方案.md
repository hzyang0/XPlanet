...
> **历史文档说明（2026-07-20）**：本文记录 v1/v2 从社区后端演进到首个 Agent 闭环的设计和实施证据，不再作为后续目标架构。后续唯一实施基线为 [`XPlanet-Agent-First整体重构方案.md`](XPlanet-Agent-First整体重构方案.md)。未迁移完成前，本文的“已实现”内容仍用于描述当前代码事实。
>
> 文档状态：架构基线 v2.6（实施中）
> 基线日期：2026-07-16  
...
| CUR-P1-02 | 缓存重建锁显式 lease=3s，旧文档却称看门狗续期 | 慢回源超过租约后可能出现并发重建 | 已改用看门狗语义的两参数 `tryLock`，并以单元测试锁定重载选择 |
| CUR-P1-03 | 文章事务内先删缓存，提交后的 MQ 发送与第二删串联 | MQ 抛异常时第二删可能不执行；回滚会造成无效删除 | 已完成立即/延迟双事件 Outbox、带租约 relay 和幂等 L1/L2 广播失效；事务回滚仅产生无害缓存 miss |
